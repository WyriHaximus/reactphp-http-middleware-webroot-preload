...
ifeq ("$(IN_DOCKER)","TRUE")
	...
	DOCKER_RUN:=docker run --rm -it \
		...
		-w "`pwd`" \
		"ghcr.io/wyrihaximusnet/php:${PHP_VERSION}-nts-buster-dev"
endif
...
cs-fix: ## Fix any automatically fixable code style issues
	$(DOCKER_RUN) vendor/bin/phpcbf --parallel=$(shell nproc) --cache=./.phpcs.cache.json --standard=./phpcs.xml || $(DOCKER_RUN) vendor/bin/phpcbf --parallel=$(shell nproc) --cache=./.phpcs.cache.json --standard=./phpcs.xml || $(DOCKER_RUN) vendor/bin/phpcbf --parallel=$(shell nproc) --cache=./.phpcs.cache.json --standard=./phpcs.xml -vvvv

cs: ## Check the code for code style issues
	$(DOCKER_RUN) vendor/bin/phpcs --parallel=$(shell nproc) --cache=./.phpcs.cache.json --standard=./phpcs.xml
...
unit-testing: ## Run tests
	$(DOCKER_RUN) vendor/bin/phpunit --colors=always -c ./phpunit.xml --coverage-text --coverage-html ./tests-unit-coverage-html --coverage-clover ./tests-unit-clover-coverage.xml
	$(DOCKER_RUN) test -n "$(COVERALLS_REPO_TOKEN)" && test -n "$(COVERALLS_RUN_LOCALLY)" && test -f ./tests-unit-clover-coverage.xml && vendor/bin/php-coveralls -v --coverage_clover ./build/logs/clover.xml --json_path ./tests-unit-clover-coverage-upload.json || true
